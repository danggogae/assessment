(function () {
    ...
    function removeAllChildren(element) {
        while (element.firstChild) { //子の要素がある限り削除する
    ...
    //assessmentButton.onclick = function() ｛　　//無名関数の呼び出し方（その１）
    assessmentButton.onclick = () => {   //無名関数の呼び出し方（その２）
        const userName = userNameInput.value;
        if (userName.length === 0) {　//何も入力されてない場合は処理をしないで終了
            return;
        } //この処理をガード句という
    ...
    const answers = [
        ...
        '{userName}のいいところは優しさです。{userName}の優しい雰囲気や立ち振舞いに多くの人が癒やされています。'
    ];
    /**
     * 名前の文字列を渡すと診断結果を返す関数
    ...
    console.assert(
        assessment('太郎冠者') === '太郎冠者のいいところは気配りです。太郎冠者の配慮が多くの人を救っています。',
        '診断結果の文言の特定の部分を名前に置き換える処理が正しくありません！'
    );

    console.assert(
        assessment('太郎冠者') === assessment('太郎冠者'),
        '処理が正しくありません！'
    );
